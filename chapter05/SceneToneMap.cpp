...
void SceneToneMap::compileAndLinkShader()
{
    try {
#ifdef __APPLE__
        prog.compileShader("shader/tonemap_41.vert");
        prog.compileShader("shader/tonemap_41.frag");
#else
        prog.compileShader("shader/tonemap.vert");
        prog.compileShader("shader/tonemap.frag");
#endif
        prog.link();
        prog.use();
