...
void SceneSamplerObj::initScene()
{
    ...
    GLubyte *data = new GLubyte[w * h * checkSize];
    for (int r = 0; r < h; r++) {
        for (int c = 0; c < w; c++) {
            GLubyte color = 0;
    ...
    // Bind texture object and sampler object to texture uint
...
void SceneSamplerObj::compileAndLinkShader()
{
    try {
#ifdef __APPLE__
        prog.compileShader("shader/texture_41.vert");
        prog.compileShader("shader/texture_41.frag");
#else
