...
void SceneMultiTex::compileAndLinkShader()
{
    try {
#ifdef __APPLE__
        prog.compileShader("shader/multitex_41.vert");
        prog.compileShader("shader/multitex_41.frag");
#else
        prog.compileShader("shader/multitex.vert");
        prog.compileShader("shader/multitex.frag");
#endif
        prog.link();
        prog.use();
