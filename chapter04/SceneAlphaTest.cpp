...
void SceneAlphaTest::compileAndLinkShader()
{
    try {
#ifdef __APPLE__
        prog.compileShader("shader/alphatest_41.vert");
        prog.compileShader("shader/alphatest_41.frag");
#else
        prog.compileShader("shader/alphatest.vert");
        prog.compileShader("shader/alphatest.frag");
#endif
        prog.link();
        prog.use();
